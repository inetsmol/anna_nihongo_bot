...
def openai_gpt_add_space(text):
    if location == 'ja-JP':
        ...
        response = client.responses.create(
            model="gpt-5-nano",
            input=f"add spaces between words in the following text {text} return only spaced text"
        )
...
def openai_gpt_translate(text):
    ...
    response = client.responses.create(
        model="gpt-5-nano",
        input=f"translate this text into Russian: {text}. answer only with translation"
    )
    return response.output_text
...
def openai_gpt_get_phrase_from_text(text):
    ...
    response = client.responses.create(
        model="gpt-5-nano",
        input=f"выбери из текста 5 фраз из 2-3 слов содержащих прилагательные и глаголы. фразы не "
